...
class GeminiService(
    ...
    fun generateResultMessage(request: LlmRequest): String {
        val prompt = if (request.contextType == "RESULT_COMPARISON") {
            "사용자가 방금 '${request.activityType}' 활동을 마치고 왔어. 활동 전후로 졸림 수치가 ${request.sleepinessDiff}만큼 개선되었어. 이 변화를 칭찬하고 남은 하루도 파이팅하라는 다정한 코칭 멘트를 딱 1문장(최대 50자 이내)으로 만들어줘. 이모지도 하나 섞어줘."
        } else {
        ...
        return try {
            val response = webClient.post()
                ...
                .retrieve()
                .bodyToMono(GeminiResponse::class.java)
                .timeout(Duration.ofSeconds(6)) // 6초 방어막 동일 적용
                .block()
